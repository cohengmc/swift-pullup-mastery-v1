...
struct SubMaxVolumeView: View {
    ...
    @State private var showingSetComplete = false
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Set progress at top (matching second image layout - 2x5 grid)
            SubMaxSetProgressView(
            ...
            if currentSet <= totalSets {
                if isRestingBetweenSets {
                    ...
                    VStack(spacing: 40) {
                        ...
                        SimpleTimer(initialTime: restTime) {
                            withAnimation {
                    ...
                    // Active set phase (matching second image)
                    VStack(spacing: 40) {
                        VStack(spacing: 8) {
                            Text("50%")
                        ...
                        VStack(spacing: 16) {
                            ...
                            NumberWheel(selectedValue: $currentReps, minValue: 0, maxValue: max(30, targetReps + 10))
                        }
                    }
                }
            } else {
    ...
    private func completeCurrentSet() {
        ...
        // Start rest immediately if not the last set
        if currentSet < totalSets {
            withAnimation {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 16) {
                ...
                NumberWheel(selectedValue: $selectedReps, minValue: 0, maxValue: max(30, targetReps + 10))
            }
